...
fun LoginScreen(
    ...
    // При успешном входе — сохраняем ID сотрудника и переходим в приложение
    // Демо: используем ID=1. В продакшене — после авторизации через Sber ID
    fun onLogin() {
        UserSession.login(1L)
        navHostController.navigate(Screen.Status.route) {

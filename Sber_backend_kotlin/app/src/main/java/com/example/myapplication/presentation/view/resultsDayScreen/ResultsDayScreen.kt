...
import com.example.myapplication.presentation.viewmodel.ResultsDayViewModel
import com.example.myapplication.presentation.viewmodel.SubmitState
import com.example.myapplication.presentation.viewmodel.TodayState
import com.example.myapplication.ui.theme.*

@Composable
fun ResultsDayScreen(
    navHostController: NavHostController,
    viewModel: ResultsDayViewModel = viewModel()
) {
    val todayState by viewModel.todayState.collectAsState()
    val submitState by viewModel.submitState.collectAsState()

    LaunchedEffect(Unit) { viewModel.loadTodayResults() }
    ...
    LaunchedEffect(todayState) {
        when (val s = todayState) {
            is TodayState.Success -> {
                if (s.dealsCount > 0 || s.volumeAmount > 0 || s.productsCount > 0) {
                    dealsCount = s.dealsCount.toString()
                    creditVolume = s.volumeAmount.toString()
                    productsCount = s.productsCount.toString()
                }
            }
            else -> {}
        }
    ...
    Column(
        ...
        // Нижняя кнопка: сохранить или перейти к детализации
        Box(modifier = Modifier.padding(20.dp).padding(bottom = 10.dp)) {
            Button(
                onClick = {
                    val d = dealsCount.toIntOrNull() ?: 0
                    val v = creditVolume.replace(',', '.').toDoubleOrNull() ?: 0.0
                    val p = productsCount.toIntOrNull() ?: 0
                    if (d > 0 || v > 0 || p > 0) {
                        viewModel.submit(d, v, p)
                    } else {
                        navHostController.navigate(Screen.Details.route)
                    }
                },
                ...
                Text(
                    text = when (submitState) {
                        is SubmitState.Loading -> "Сохранение..."
                        is SubmitState.Success -> "Сохранено!"
                        else -> "Сохранить и детализация рейтинга"
                    },
                    fontSize = 16.sp,
